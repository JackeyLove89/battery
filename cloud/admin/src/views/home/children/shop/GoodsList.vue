<template>
  <div>
    <!-- 导航 -->
    <mbx-nav one-nav="车辆电池信息" two-nav="电池列表"></mbx-nav>
    <!-- 卡片试图 -->
    <el-card>
      ...
      <el-table :data="dataList" stripe border class="user-table">
	   ...
	   <el-table-column label="车辆图片" prop="avatarurl">
	     <template v-slot="scope">
	      <img  v-image-preview :src="getImgUrl(scope.row.car.img)"  class="el-avatar"/>
	     </template>
	     </el-table-column>
	
		<el-table-column prop="car.name" label="车辆名称"></el-table-column> 
	
	   <el-table-column prop="car.type" label="电池品牌"></el-table-column>	
		 <el-table-column prop="car.dec" label="续航公里数(Km)"></el-table-column>	
	    	
        <el-table-column prop="jkdb" label="健康度(%)"></el-table-column>	  
		<el-table-column prop="rlb" label="容量保持率(%)"></el-table-column>	
		<el-table-column prop="dyb" label="电压一致性(%)"></el-table-column>
		<el-table-column prop="nzb" label="内阻一致性(%)"></el-table-column>
		<el-table-column prop="wdb" label="温度一致性(%)"></el-table-column>
		<el-table-column prop="zfdb" label="自放电率(%)"></el-table-column>
		<el-table-column prop="xh" label="续航(km)"></el-table-column>
		<el-table-column prop="cdcs" label="充电次数"></el-table-column>
		<el-table-column prop="xslc" label="总里程(km)"></el-table-column>
		<el-table-column prop="zgwd" label="最高温度(°)"></el-table-column>
		<el-table-column prop="zdwd" label="最低温度(°)"></el-table-column>
...
export default {
  ...
  upTypeChange(e){
   ...
   getDicList(){		
    ...
    getList(){		
      getDList(this.queryInfo).then((res) => {
    ...
    addClick(){     
      this.$refs.addForm.validate((reg) => {
        if(reg){
          // 验证成功发（请求）
          // 验证成功发（请求）
          addLrInfo(this.addForm).then((res) => {
            // console.log(res)
            if(res.data.code === 200){
              this.addInfoShow = false
    ...
    changeClick(){     
     ...
     this.$refs.changeForm.validate((reg) => {
        if(reg){
          // 验证成功发（请求）
          // 验证成功发（请求）
          upLrInfo(this.changeForm).then((res) => {
            // console.log(res)
            if(res.data.code === 200){
              this.changeUserShow = false
              this.$message.success(res.data.msg)
...
/**
    ...
    delClick(userInfo){
		  ...
		  this.$confirm('您确定要永久删除吗？', '永久删除' ,{
		     ...
		     * 删除请求模块
		     */
		    delLrInfo(userInfo.id).then((res) => {
		      if(res.data.code === 200){
